"""End-to-end authority and adoption tests for formula-safe XLSX research."""
...
SKILL_ID = "clicky.research_to_xlsx"
VERIFIER_ID = "research-xlsx-v1"
...
class BundledResearchXlsxSkillTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
    ...
    def _run(self, inputs: dict[str, object], *, run_id: str) -> TaskRun:
        ...
        return TaskRun(
            TaskSpec(
                ...
                goal="Create a reviewed source-backed research XLSX.",
                input_digest=declarative_inputs_digest(
                    definition,
                    inputs,
                ),
                requested_result="A verified local XLSX artifact.",
    ...
    def test_bundle_is_integrity_bound_opt_in_and_has_no_external_write(self):
        ...
        self.assertFalse(
            declarative_skill_invocation_allowed(
                ...
                SKILL_ID,
                explicit_selection=False,
                utterance="research these results to xlsx",
            )
        )

    async def test_bounded_research_waits_on_exact_validated_xlsx_preview(self):
        inputs = {
        ...
        self.assertEqual(payload.target.reference, "research-xlsx")
        self.assertEqual(payload.target.label, "research.xlsx")
        self.assertEqual(payload.preview.media_type, XLSX_MEDIA_TYPE)
        self.assertIn("\"columns\":[\"entity\"", payload.preview.excerpt)
        self.assertIn("\"row_count\":2", payload.preview.excerpt)
        self.assertNotIn("PK", payload.preview.excerpt)
        ...
        self.assertEqual(artifact.name, "research.xlsx")
        ...
        validate_safe_xlsx_package(adopted_files[0].read_bytes())
