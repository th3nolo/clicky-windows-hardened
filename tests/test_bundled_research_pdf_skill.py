"""End-to-end authority and adoption tests for inert PDF reports."""
...
SKILL_ID = "clicky.research_to_pdf"
...
class BundledResearchPdfSkillTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
    ...
    def _run(self, inputs: dict[str, object], *, run_id: str) -> TaskRun:
        ...
        return TaskRun(
            TaskSpec(
                ...
                goal="Create a reviewed source-backed PDF report.",
                input_digest=declarative_inputs_digest(
                    definition,
                    inputs,
                ),
                requested_result="A verified inert local PDF artifact.",
    ...
    def test_bundle_has_no_external_write_or_document_automation_authority(
        self,
    ):
        definition = self.definition
        self.assertEqual(
    ...
    async def test_exact_visible_preview_is_verified_then_adopted(self):
        inputs = {
        ...
        self.assertEqual(payload.target.reference, "research-pdf")
        self.assertEqual(payload.target.label, "research.pdf")
        self.assertEqual(payload.preview.media_type, PDF_MEDIA_TYPE)
        ...
        self.assertNotIn("%PDF", payload.preview.excerpt)
        self.assertGreater(
        ...
        self.assertEqual(artifact.name, "research.pdf")
        self.assertEqual(artifact.media_type, PDF_MEDIA_TYPE)
        self.assertEqual(
            artifact.sha256,
        ...
        document = validate_safe_pdf_document(adopted[0].read_bytes())
        self.assertGreaterEqual(document.page_count, 1)
        self.assertEqual(document.paragraphs[0].text, "Creator research")

    async def test_shortfall_or_invented_source_never_reaches_pdf(self):
        inputs = {
            "query": "public creator profiles",
            "report_title": "Creator research",
            "requested_sections": 2,
