"""End-to-end authority and adoption tests for Markdown research reports."""
...
SKILL_ID = "clicky.research_to_markdown"
...
class BundledResearchMarkdownSkillTests(
    ...
    def _run(self, inputs: dict[str, object], *, run_id: str) -> TaskRun:
        ...
        return TaskRun(
            TaskSpec(
                ...
                goal="Create a reviewed source-backed research report.",
                input_digest=declarative_inputs_digest(
                    definition,
                    inputs,
                ),
                requested_result=(
                    "A verified local Markdown report artifact."
                ),
    ...
    def test_bundle_has_only_bounded_research_and_local_write_authority(self):
        definition = self.definition
        self.assertEqual(
    ...
    async def test_exact_cited_report_preview_is_verified_then_adopted(self):
        inputs = {
        ...
        runner, run, workspace = self._runner(
            inputs,
            run_id="bundled-markdown-complete",
        ...
        self.assertEqual(payload.target.reference, "research-markdown")
        self.assertEqual(payload.target.label, "research.md")
        self.assertEqual(payload.preview.media_type, "text/markdown")
        ...
        self.assertEqual(payload.preview.byte_count, len(preview_bytes))
        self.assertEqual(
        ...
        self.assertEqual(artifact.name, "research.md")
    ...
    async def test_shortfall_or_invented_source_never_reaches_approval(self):
        ...
        short_runner, short_run, short_workspace = self._runner(
            inputs,
            run_id="bundled-markdown-shortfall",
        ...
        invented_runner, invented_run, invented_workspace = self._runner(
            inputs,
            run_id="bundled-markdown-invented",
