...
The request format was checked against [OpenRouter's video-input documentation](https://openrouter.ai/docs/guides/overview/multimodal/videos). Live synthetic tests on 2026-09-14 identified changing visual equations, but neither Muse selection demonstrated understanding of the spoken-only word inside the MP4. The recording dialog discloses that limitation. Explicit audio input to Muse 1.2 did transcribe the phrase; that does not establish embedded-audio or physical microphone readiness. See [workflow acceptance](WORKFLOW_ACCEPTANCE.md).
